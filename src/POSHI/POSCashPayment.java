...
public class POSCashPayment extends JPanel {
	 ...
	 */
	public POSCashPayment(JFrame currentFrame, Store store, Session session,Sale sale, JTextField saleTotal,JTextField AmtTendered,JButton btnPaymentComplete) {
		setLayout(null);
	    ...
	    btnSave.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				Cash cash = new Cash(sale.calcAmount(new BigDecimal(textField_2.getText())),new BigDecimal(textField_2.getText()));
				session.getRegister().getCashDrawer().addCash(sale.calcAmount(new BigDecimal(textField_2.getText())));
				sale.addPayment(cash);
				
				System.out.println(new BigDecimal(textField_2.getText()).toString());
				saleTotal.setText(sale.calcAmount(new BigDecimal(textField_2.getText())).toString());
